...
def populate_database(db: Session, users, genres, games):
    ...
    # Add genres
    for genre in genres:
        new_genre = GameGenre(name=genre["name"], description=genre["description"])
        game_genre_crud.create(db, new_genre)

    # Add games
    for game in games:
        new_game = Game(name=game["name"], description=game["description"])
        new_game_id = game_crud.add_game(db, new_game)
        game_genre_association_crud.add_association(db, game_id=new_game_id, genre_id=game["genre_list"])


if __name__ == "__main__":
    populate_database(db = db_setup.get_db, users=users, genres=genres, games=games)
