...
from user_profile_module import crud_user
...
def user_login(user: UserLogin, db: Session = Depends(db_setup.get_db)):
    if crud_user.authenticate(db=db, user=user):
        if crud_user.get_user_by_email(db=db, email=user.email).is_active is False:
            raise HTTPException(
...
def read_current_user(db: Session = Depends(db_setup.get_db), current_user_email: str = Depends(jwt_scheme)):
    db_user = crud_user.get_user_by_email(
...
def delete_current_user(response: Response, db: Session = Depends(db_setup.get_db), current_user_email: str = Depends(jwt_scheme)):
    db_user = crud_user.get_user_by_email(db=db, email=current_user_email)
    if db_user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User with this email does not exists."
        )
    crud_user.delete_user(db=db, email=current_user_email)
    response.delete_cookie(key="Authorization")
